...
public class ProductCompositionController : AdminController, IProductCompositionService
{
    ...
    private readonly IRequestClient<UpdateProduct> _updateProductRequestClient;

    public ProductCompositionController(
        IMediator mediator, IRequestClient<CreateProduct> createProductRequestClient, IRequestClient<UpdateProduct> updateProductRequestClient)
    {
    ...
    [HttpPut]
    //[Authorize("UpdateProductComposition")]
    public async Task<ProductCompositionDto> UpdateAsync(Guid id , UpdateProductCompositionDto input)
    {
        try
        {
            Response response = await _updateProductRequestClient.GetResponse<ProductUpdated, ProductUpdateFaulted>(new
            {
                ProductId = id,
                Name = input.Name,
                Description = input.Description,
            ...
            return response switch
            {
                (_, ProductUpdated completed) => completed.Product,
                (_, ProductUpdateFaulted notCompleted) => throw new UserFriendlyException(notCompleted.Reason),
                _ => throw new Exception("unknown error has happened")
            };
        }
        catch (RequestTimeoutException)
